...
public class AptitudeCapability {
    ...
    private Map<String, Integer> mapAptitudes() {
        ...
        List<Aptitude> aptitudeList = RegistryAptitudes.APTITUDES_REGISTRY.getValues().stream().toList();
    ...
    private Map<String, Integer> mapPassive() {
        Map<String, Integer> map = new HashMap<>();
        List<Passive> passiveList = RegistryPassives.PASSIVES_REGISTRY.getValues().stream().toList();
    ...
    private Map<String, Boolean> mapSkills() {
        ...
        List<Skill> skillList = RegistrySkills.SKILLS_REGISTRY.getValues().stream().toList();
    ...
    private Map<String, Boolean> mapTitles() {
        Map<String, Boolean> map = new HashMap<>();
        List<Title> titleList = RegistryTitles.TITLES_REGISTRY.getValues().stream().toList();
    ...
    public static AptitudeCapability get(Player player) {
        if (player instanceof AptitudeCapabilityHolder holder) {
            return holder.justlevelingfork$getAptitudeCapability();
        }
        return null;
    }
    ...
    public int getPassiveLevel(Passive passive) {
        return RegistryPassives.isEnabled(passive) ? this.passiveLevel.getOrDefault(passive.getName(), 0) : 0;
    }
    ...
    public boolean getToggleSkill(Skill skill) {
        return RegistrySkills.isEnabled(skill) && this.toggleSkill.getOrDefault(skill.getName(), false);
    }

    public void setToggleSkill(Skill skill, boolean toggle) {
        if (!RegistrySkills.isEnabled(skill)) {
    ...
    public boolean canUseBlock(Player player, Block block) {
        return canUse(player, Objects.requireNonNull(BuiltInRegistries.BLOCK.getKey(block)));
    }

    public boolean canUseEntity(Player player, Entity entity) {
        return canUse(player, Objects.requireNonNull(BuiltInRegistries.ENTITY_TYPE.getKey(entity.getType())));
    }
    ...
    public CompoundTag serializeNBT() {
        ...
        for (Title title : RegistryTitles.TITLES_REGISTRY.getValues().stream().toList()){
