    ...
    public void onHudRender(Post event) {
        ...
        if (this.client.level != null && this.client.player != null && showTicks >= 0 && this.client.player.getCapability(RegistryCapabilities.APTITUDE).isPresent()) {
            if (list.count() > 0) {
                ...
                matrixStack.scale(scale2, scale2, 1.0F);
                ...
                Utils.drawCenterWithShadow(matrixStack, new TranslatableComponent("overlay.title.you_gain_a_title"), xOff2, yOff2 - 10, Color.WHITE.getRGB());
                matrixStack.popPose();

                float scale1 = 0.1F * scaleTick;
                matrixStack.pushPose();
                matrixStack.scale(scale1, scale1, 1.0F);
                ...
                Utils.drawCenterWithShadow(matrixStack, new TranslatableComponent("overlay.title.format", new TranslatableComponent(getTitle.getKey()).withStyle(ChatFormatting.BOLD)), xOff1, yOff1, Color.WHITE.getRGB());
                matrixStack.popPose();
                if (showTicks == 0) {
                    list.dequeue();
