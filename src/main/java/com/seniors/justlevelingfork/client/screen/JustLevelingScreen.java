 ...
 public class JustLevelingScreen extends Screen {
   ...
   public void drawBackground(GuiGraphics matrixStack, int x, int y, int mouseX, int mouseY, float delta) {
     ...
     if (this.selectedPage == 1) {
       RenderSystem.enableBlend();
       if ((RegistryAptitudes.getAptitude(this.selectedAptitude)).background != null) matrixStack.blit((RegistryAptitudes.getAptitude(this.selectedAptitude)).background, x + 7, y + 30, 0.0F, 0.0F, 160, 128, 16, 16); 
   ...
   public void drawTitles(GuiGraphics matrixStack, int x, int y, int mouseX, int mouseY, float delta) {
     ...
     for (Title titles : titleList) {
       if (titles.getRequirement()) {
         unlockTitles.add(titles); continue;
       } 
     ...
     if (this.scrollDropDown > searchTitleList.size() - maxSize) this.scrollDropDown = searchTitleList.size() - maxSize; 
   ...
   public void drawAptitudes(GuiGraphics matrixStack, int x, int y, int mouseX, int mouseY) {
     ...
     for (int i = 0; i < aptitudeList.size(); i++) {
       ...
       if (Utils.checkMouse(xPos, yPos, mouseX, mouseY, 74, 26)) matrixStack.blit(HandlerResources.SKILL_PAGE[this.selectedPage], xPos, yPos, 176, 0, 73, 26);
   ...
   public void drawSkills(GuiGraphics matrixStack, int x, int y, int mouseX, int mouseY) {
     ...
     switch (HandlerConfigClient.sortPassive.get()) { case ByName:
         listPassives.sort(new SortPassiveByName()); break;
       case ByReverseName: listPassives.sort((new SortPassiveByName()).reversed()); break; }
     
     switch (HandlerConfigClient.sortSkill.get()) { case ByName:
         listSkills.sort(new SortSkillByName()); break;
       case ByReverseName: listSkills.sort((new SortSkillByName()).reversed()); break;
       case ByLevel: listSkills.sort(new SortSkillList());
         break; }
     ...
     int i;
     for (i = 0; i < sorted.size(); ) { this.skillSize = (i - i % 5) / 5; i++; }
      for (i = 0; i <= this.skillSize; ) { this.skillSizePage = (i - i % 4) / 4; i++; }
   ...
   public void createList(List<Object> list, AptitudeCapability capability, GuiGraphics matrixStack, int x, int y, int mouseX, int mouseY) {
     for (int i = 0; i < list.size(); i++) {
       ...
       Object object = list.get(i); if (object instanceof Passive passive) {
       ...
       object = list.get(i); if (object instanceof Skill skill) {
         int isToggle = 0;
