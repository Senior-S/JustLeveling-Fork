...
public class Aptitude extends ForgeRegistryEntry<Aptitude> {
    ...
    public String getKey() {
        return String.format("aptitude.%s.%s", this.key.getNamespace(), this.key.getPath());
    }
    ...
    public MutableComponent getRank(int aptitudeLevel) {
        MutableComponent rank = new TranslatableComponent("aptitude.justlevelingfork.rank.0");
        for (int i = 0; i < 9; i++) {
            if (aptitudeLevel >= (HandlerCommonConfig.HANDLER.instance().aptitudeMaxLevel / 8) * i) {
                rank = new TranslatableComponent("aptitude.justlevelingfork.rank." + i);
            }
        }
        return rank;
    }
